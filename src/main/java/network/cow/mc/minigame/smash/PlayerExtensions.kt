...
fun Player.knockback(direction: Vector, power: Double) {
    ...
    network.cow.mc.minigame.smash.BummsTask(
        this,
...
fun Player.getHitter(): network.cow.mc.minigame.smash.Hitter? {
    return this.getSmashState(StateKey.HITTER)
}

fun Player.setHitter(hitter: network.cow.mc.minigame.smash.Hitter?) {
    if (hitter == null) {
